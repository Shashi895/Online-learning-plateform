...
const addLectureToCourseById = async (req, res, next) => {
  try {
    ...
    if (!videoFile || !pdfFile) {
      return next(new AppError("Both video and PDF are required", 400));
    }
    ...
    lectureData.lecture.public_id = videoUpload.public_id;
    lectureData.lecture.secure_url = videoUpload.secure_url;
    ...
    lectureData.pdf.public_id = pdfUpload.public_id;
    lectureData.pdf.secure_url = pdfUpload.secure_url;
    ...
    res.status(200).json({
      success: true,
      message: "Lecture with video and PDF added successfully",
    });
  } catch (e) {
    return next(new AppError(e.message, 500));
  }
